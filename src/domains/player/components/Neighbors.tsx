      ...
      {neighborFactions.map((neighbor, index) =>
        neighbor.faction ? (
          ...
          <Box
            key={index}
            title={
              findColorData(neighbor.color)?.displayName ??
              findColorData(neighbor.color)?.name ??
              neighbor.color
            }
            className={styles.unidentifiedNeighborDot}
            style={{ background: getPrimaryColorCSS(neighbor.color) }}
          />
        )
      )}
    </Group>
